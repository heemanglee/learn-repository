...
스레드를 사용할 클래스에서 Thread 클래스를 상속하여 run() 메서드룰 오버라이딩한다.
```java
public class Main {
...
멀티 스레드로 동작하기 때문에 main 스레드에서 생성된 5개의 스레드들은 새로운 스택 영역을 할당받아 데이터를 관리한다. 참고로 메인 스레드는 5개의 스레드를 생성 및 실행하는 것과 콘솔에 출력하는 작업을 완료하면 main 스래드에게 할당된 호출 스택이 제거된다.
